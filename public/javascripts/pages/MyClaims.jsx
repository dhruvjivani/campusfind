...
function MyClaims({ onNavigate, user }) {
    ...
    <div className="container">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <h3>No claims yet</h3>
          <p>You haven't submitted any claims. Browse items to find something that's yours.</p>
          ...
          {claims.map(claim => (
            <div key={claim.id} className="card">
              ...
              {statusNote[claim.status] && (
                <div className={`alert ${claim.status === 'verified' || claim.status === 'completed' ? 'success' : claim.status === 'rejected' ? 'error' : 'info'}`}
              ...
              {claim.verification_notes && (
                <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-sm)', padding: '10px 12px', marginBottom: 10, fontSize: '0.87rem' }}>
              ...
              <div className="card-meta">
                <span className="card-meta-item">📅 Submitted {new Date(claim.created_at).toLocaleDateString()}</span>
              </div>

              <div className="card-actions">
                {claim.item_id && (
                  <button className="btn-secondary" onClick={() => onNavigate('itemdetail', claim.item_id)}>
                    View Item
                  </button>
                )}
                {claim.status === 'pending' && (
                  ...
                  >
                    {cancelling[claim.id] ? '...' : '🗑 Cancel Claim'}
                  </button>
                )}
              </div>
            </div>
