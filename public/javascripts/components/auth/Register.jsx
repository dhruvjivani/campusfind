...
function Register({ onNavigate, onLoginSuccess }) {
  ...
  const getPasswordStrength = (pw) => {
    ...
    return { score, label: labels[score], cls: classes[score], bars: [score>=1, score>=2, score>=3, score>=4] };
  };
        ...
        <div className="auth-brand-panel">
          ...
          <div className="auth-features">
            <div className="auth-feature-item">
              <span>⚡</span>
              <span>Free to use — takes 30 seconds</span>
            </div>
            <div className="auth-feature-item">
              <span>🔐</span>
            ...
            <div className="auth-feature-item">
              <span>📬</span>
              <span>Get notified when your item is found</span>
            </div>
            <div className="auth-feature-item">
              <span>🏫</span>
              <span>Conestoga College campuses supported</span>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              {formData.email.endsWith('.on.ca') && (
                <p style={{ fontSize: '0.78rem', color: 'var(--success)', marginTop: '4px' }}>
                  ✅ College email — your account will be auto-verified
                </p>
              )}
            </div>
              ...
              {formData.password && (
                <div className="password-strength">
                  {strength.bars.map((filled, i) => (
                    <div key={i} className={`strength-bar${filled ? ' ' + strength.cls : ''}`}></div>
                  ))}
            ...
            <div className="form-group">
              ...
              {formData.confirmPassword && formData.password !== formData.confirmPassword && (
                <p style={{ fontSize: '0.78rem', color: 'var(--danger)', marginTop: '4px' }}>❌ Passwords do not match</p>
              )}
              {formData.confirmPassword && formData.password === formData.confirmPassword && formData.password.length >= 6 && (
                <p style={{ fontSize: '0.78rem', color: 'var(--success)', marginTop: '4px' }}>✅ Passwords match</p>
              )}
            </div>
